...
class Base:
    ...
    def get_login_account_from_config(self):
        """
    ...
    def login_by_config_url(self):
        ...
        if self.driver.name == "internet explorer":
            logging.info("正在进行IE浏览器UI自动化测试，开启跳过Https安全校验功能")
            try:
                self.skip_ie_https_check(self.driver)
            except Exception as e:
                logging.info("该网站https证书正常，不需要进行跳过操作")
    ...
    def find_element(self, locator):
        ...
        if not isinstance(locator, tuple):
            ...
            try:
                ...
                logging.warning("定位不到元素，错误信息为:{}".format(e))
                return False
    ...
    def get_all_text_by_elements(self, locator):
        ...
        try:
            ...
            logging.info("获取元素组对象列表的值成功，值为：{}".format(elems_text))
        except Exception as e:
    ...
    def get_attribute_by_value(self, locator, attribute_value):
        ...
        try:
            ...
            logging.info("该元素对象获取{}成功，{}值为：{}".format(attribute_value, attribute_value, elem_attribute_text))
        except Exception as e:
            logging.error("该元素对象获取{}失败，错误信息为：{}".format(attribute_value, e))
    ...
    def elem_is_enable(self, locator):
        ...
        """
        elem = self.find_element(locator)
        logging.info("元素状态，是否可操作：{}".format(elem.is_enabled()))
    ...
    def choose_elem_by_visible_value(self, locator, value):
        ...
        for item in elems:
            if self.elem_object_get_text(item) == value:
                logging.info("输出符合筛选条件的元素对象：{}".format(item))
                return item
    ...
    def elem_object_click(self, elem):
        ...
        @return:
        """
        logging.info("执行点击事件的元素对象为:{}".format(elem))
        try:
            elem.click()
        except Exception as e:
            logging.error("元素点击失败，错误信息为：{}".format(e))
